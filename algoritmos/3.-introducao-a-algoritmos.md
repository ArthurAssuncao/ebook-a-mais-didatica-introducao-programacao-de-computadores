...
1\) Escreva um algoritmo em português \(descrição narrativa ou portugol/português estruturado\) que consista no passo a passo das suas atividades ao se levantar até a hora do almoço nas terças-feiras.
...
* semântica: se refere ao uso lógico das palavras reservadas \(\textit{keyword}\), comandos etc, por exemplo se o uso de determinada palavra reservada faz sentido em um contexto específico ou se uma variável de um tipo está recebendo esse tipo ou outro tipo incompatível.

#### Estrutura Básica de um Algoritmo em Linguagem de Programação

A seguir, na Figura \ref{fig:diagrama-estrutura-algoritmos} é apresentado um diagrama de uma estrutura comum em algoritmos criados em linguagem de programação. É importante notar que todos programas/códigos criados a partir de agora em linguagem Scilab, C ou no Portugol Studio utilizarão uma estrutura parecida.
...
* Qual a principal diferença entre implementação de linguagem Interpreta e Compilada?
